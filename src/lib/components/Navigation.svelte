<script lang="ts">
	...
	let isMobileMuneOpen = false;
	$: isOpen = desktop || isMobileMuneOpen;
	...
	let lastFocusedElement: HTMLAnchorElement;

	const openMenu = async () => {
		isMobileMuneOpen = true;
		await tick();
		closeMenuButton.getButton().focus();
	};

	const closeMenu = async () => {
		isMobileMuneOpen = false;
		await tick();
				...
				{#each menuItems as item, index}
					...
					<li class:active={item.path === $page.url.pathname}>
						{#if menuItems.length === index + 1}
							<a bind:this={lastFocusedElement} href={item.path} on:keydown={moveFocusToTop}>
